...
class Register:
    def __init__(self,root):
        ...
        chk=tk.Checkbutton(frame1,text="Agree to Terms & Conditions",variable=self.var_chk,onvalue=1,offvalue=0,
                           bg="#161B21",fg="#F4A950",activebackground="#F4A950",activeforeground="#161B21",
                           font=("times new roman",12))
        chk.place(x=50,y=385)
        
        #--------Register & Signin Buttons------------#
        btn_register=tk.Button(frame1,text="Register Now",font=("times new roman",13),bg="#161B21",
                               fg="#F4A950",activebackground="#F4A950",activeforeground="#161B21",bd=0,
                               cursor="hand2",command=self.register_data)
        btn_register.place(x=50,y=430)
        
        btn_login=tk.Button(self.root,text="Sign In",font=("times new roman",13),bg="#161B21",fg="#F4A950",
                            activebackground="#F4A950",activeforeground="#161B21",bd=0,cursor="hand2",
                            command=self.signin)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or \
                ...
                self.txt_passwd.get()=="" or self.txt_cpasswd.get()=="":
            ...
            try:
                ...
                #print(row)
                if row!=None:
                    messagebox.showerror("Error","User already exists with same email address,"
                                                 "Register with another Email address",parent=self.root)
                else:

...
class Login_System:
    def __init__(self,root):
        ...
        txt_pass=tk.Entry(login_frame,textvariable=self.password,show="*",font=("times new roman",15),bg="#1D1B1B",
                          fg="White")
        txt_pass.place(x=50,y=240,width=250)
        
        btn_login=tk.Button(login_frame,command=self.login,text="Log In",font=("times new roman",15),bg="#1D1B1B",
                            activebackground="#EC4D37",fg="#EC4D37",activeforeground="#1D1B1B",cursor="hand2")
        ...
        btn_forget=tk.Button(login_frame,text="Forget Password?",command=self.forget_password_window,
                             font=("times new roman",13),bg="#1D1B1B",fg="#EC4D37",bd=0,activebackground="#EC4D37",
                             activeforeground="#1D1B1B")
        ...
        btn_signup=tk.Button(register_frame,text="Sign Up",command=self.signup,font=("times new roman",13,"bold"),
                             bg="#1D1B1B",fg="#EC4D37",bd=0,activebackground="#EC4D37",activeforeground="#1D1B1B")
    ...
    def forget_password(self):
        if self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_new_pass.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from regfields where email=%s and question=%s and answer=%s",
                ...
                if row==None:
                    ...
                    cur.execute("update regfields set password=%s where email=%s",(self.txt_new_pass.get(),
                                                                                   self.email.get()))
                    con.commit()
                    con.close()
        ...
        if self.email.get()=="":
            ...
            try:
                ...
                if row==None:
                    ...
                    t=tk.Label(self.root2,text="Forget Password",font=("times new roman",20,"bold"),
                               bg="#1D1B1B",fg="#EC4D37")
                    ...
                    question=tk.Label(self.root2,text="Security Question",font=("times new roman",15,"bold"),
                                      bg="#1D1B1B",fg="#EC4D37")
                    ...
                    answer=tk.Label(self.root2,text="Answer",font=("times new roman",15,"bold"),
                                    bg="#1D1B1B",fg="#EC4D37")
                    answer.place(x=50,y=180)
                    
                    self.txt_answer=tk.Entry(self.root2,font=("times new roman",15),bg="#1D1B1B",
                                             fg="White")
                    self.txt_answer.place(x=50,y=210,width=250)
                    
                    new_pass=tk.Label(self.root2,text="New Password",font=("times new roman",15,"bold"),
                                      bg="#1D1B1B",fg="#EC4D37")
                    ...
                    btn_change_password=tk.Button(self.root2,text="Reset Password",command=self.forget_password,
                                                  bg="#1D1B1B",fg="#EC4D37",activebackground="#EC4D37",
                                                  activeforeground="#1D1B1B",font=("times new roman",15,"bold"))
    ...
    def login(self):
        if self.email.get()=="" or self.password.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost",user="root",password="",database="ems")
                cur=con.cursor()
                cur.execute("select * from regfields where email=%s and password=%s",(self.email.get(),
                                                                                      self.password.get()))
                row=cur.fetchone()
                if row==None:
